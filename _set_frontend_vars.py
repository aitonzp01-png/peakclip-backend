...
for name, value in vars:
    mutation = 'mutation { variableUpsert(input: { environmentId: "%s", projectId: "%s", serviceId: "%s", name: "%s", value: "%s" }) }' % (env_id, project_id, service_id, name, value)
    r = requests.post("https://api.railway.app/graphql/v2", json={"query": mutation}, headers=headers, timeout=30)
    result = r.json()
    if "errors" in result:
        print(f"FAIL {name}: {result['errors'][0]['message'][:80]}")
    else:
        print(f"OK   {name}")
